...
//if( fDebug ){ printf("IsMonitorTx:: [%s]\n", sFile.c_str()); }
	string sTxMsg = tx.chaindata.c_str();	// BitNet AdBonus:
		...
		if( node != NULL )
		{
			...
			if( bAdd == 2 )
			{
				if( pTalk != NULL ){
					...
					vpnNotificator->notify(Notificator::Information, qFrm, qMsg);
					//vpnNotificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid LuckChain address or malformed URI parameters."));
				}
