...
static const unsigned int MAX_TX_DATA      = 6291456;  // 6MB
static const unsigned int MAX_TX_DATA_FROM = 1280;
static const unsigned int MAX_TX_DATA_SUBJ = 1280;
static const unsigned int MAX_TX_DATA_MSG  = MAX_TX_DATA - 2560;

/** The maximum allowed size for a serialized block, in bytes (network rule) */
static const unsigned int MAX_BLOCK_SIZE = 10485760;   // 10MB
static const unsigned int MAX_BLOCK_SIZE_OLD = MAX_BLOCK_SIZE;
/** The maximum size for mined blocks */
static const unsigned int MAX_BLOCK_SIZE_GEN = MAX_BLOCK_SIZE/2;
static const unsigned int MAX_BLOCK_SIZE_GEN_OLD = MAX_BLOCK_SIZE_OLD/2;
/** The maximum size for transactions we're willing to relay/mine **/
static const unsigned int MAX_STANDARD_TX_SIZE = 9961472;  // 9.5MB
static const unsigned int MAX_STANDARD_TX_SIZE_OLD = MAX_STANDARD_TX_SIZE;
...
inline unsigned int max_BLOCK_SIZE(int64_t nHeight) { 
	...
	else return MAX_BLOCK_SIZE_OLD;
}
inline unsigned int max_BLOCK_SIZE_GEN(int64_t nHeight) { 
    if( nHeight >= NewTxFee_RewardCoinYear_Active_Height ){ return MAX_BLOCK_SIZE_GEN; }
	else return MAX_BLOCK_SIZE_GEN_OLD;
}
inline unsigned int max_STANDARD_TX_SIZE(int64_t nHeight) { 
    if( nHeight >= NewTxFee_RewardCoinYear_Active_Height ){ return MAX_STANDARD_TX_SIZE; }
	else return MAX_STANDARD_TX_SIZE_OLD;
}
inline unsigned int max_BLOCK_SIGOPS(int64_t nHeight) { 
    if( nHeight >= NewTxFee_RewardCoinYear_Active_Height ){ return MAX_BLOCK_SIGOPS; }
	else return MAX_BLOCK_SIGOPS_OLD;
}
inline unsigned int max_ORPHAN_TRANSACTIONS(int64_t nHeight) { 
...
public:
    ...
    */
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(hash=%s, nTime=%d, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%d, chaindata=%s)\n",
            GetHash().ToString().substr(0,10).c_str(),
            nTime,
            nVersion,
            vin.size(),
            vout.size(),
            nLockTime, chaindata.c_str());
...
public:
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(nprev=%p, pnext=%p, nFile=%u, nBlockPos=%-6d nHeight=%"PRIu64", nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016"PRIx64", nStakeModifierChecksum=%08x, hashProof=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
...
public:
    ...
    (
        ...
        READWRITE(nHeight);
        READWRITE(nMint);
        READWRITE(nMoneySupply);      READWRITE(nMoneyBurned);
        READWRITE(nFlags);
