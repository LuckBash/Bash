...
#else
    const char* pszModule = "luckchain";
#endif
    if (pex)
        return strprintf(
...
{
/*	boost::filesystem::path pathConfigFile(GetArg("-conf", "luckchain.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;
    return pathConfigFile; */
	
    string sConf = GetArg("-conf", "luckchain.conf");
    ...
    if (!streamConfig.good())	// No luckchain.conf file is OK	
	{
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No luckchain.conf file is OK
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "luckchaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong LuckChain will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("LuckChain"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
