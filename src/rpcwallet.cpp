...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new LuckChain address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current LuckChain address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <luckchainaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid LuckChain address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <luckchainaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <luckchainaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <luckchainaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <luckchainaddress> [minconf=1]\n"
            "Returns the total amount received by <luckchainaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toluckchainaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid LuckChain address: ")+s.name_);
    ...
    {
        printf("sendmany err: totalAmount(%I64u) >  nBalance(%I64u) [%s] \n", totalAmount, nBalance, strAccount.c_str());
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Account has insufficient funds");
    }
    ...
    if (!fCreated)
    {
        ...
        {
            printf("sendmany err: totalAmount(%I64u) + nFeeRequired(%I64u) >  nBalance(%I64u) [%s] \n", totalAmount, nFeeRequired, pwalletMain->GetBalance(), strAccount.c_str());
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a LuckChain address or hex-encoded public key\n"
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("luckchain-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("luckchain-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; LuckChain server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <luckchainaddress>\n"
            "Return information about <luckchainaddress>.");
...
Value getpubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "getpubkey <luckchain address>\n"
            "Return <luckchain pubkey>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <luckchainpubkey>\n"
            "Return information about <luckchainpubkey>.");
...
{
	return nBestHeight >= dwBitNetLotteryStartBlock;
}
    ...
    string& sGuessTxt, string& sLotteryAddr, string& sLotteryPrivKey, string& sMakerAddr, string& sLotteryLinkedTxid, string& sSignMsg)
{
	int rzt = 0;
	string stxData = "";
	if( tx.chaindata.length() > 0 ){ stxData = tx.chaindata.c_str(); }
...
// BitNet Lottery: | 123456 | 1 | 0 | 10000 | 500 | 234000 | 235000 | 6 | Vbq7grdv6caVBf1RoHPJZ9RuhsHqmY3bLi | WarWyAu2UsCKzjQWvKMT5vBbMypMuS2ru37QtUdNoFDySTjxH8uY | VevimYsrNKWbx2W6Bnq2qyLthcnEasLMTg | 21051c47f29dd01a828cd6197ce6563d4c184ecff3e34a0599fa2af8d6c65ef5 | H3rQLBiDTZwKB1OGFo8zs5RJr/XH2ubrIHGTCKnOdP4bHsu5KgikkBPujJWJ6VweY7ZZR19JbjH7kZ8qKI50h1k=
	if( (stxData.length() > 34) && (stxData.find(strBitNetLotteryMagic) == 0) )   //  "BitNet Lottery:"
	{
		...
		while (pch != NULL)
		{
			...
			else if( i == 7 ){ sStartBlock = pch;  dv =  atof(pch); iStartBlock = roundint64(dv); }  //strToInt64(pch); }
			else if( i == 8 ){ sEndBlock = pch; iEndBlock = strToInt64(pch); }
...
// BitNet Lottery: | 123456 | 1 | 0 | 10000 | 500 | 234000 | 235000 | 6 | Vbq7grdv6caVBf1RoHPJZ9RuhsHqmY3bLi | WarWyAu2UsCKzjQWvKMT5vBbMypMuS2ru37QtUdNoFDySTjxH8uY | VevimYsrNKWbx2W6Bnq2qyLthcnEasLMTg | 21051c47f29dd01a828cd6197ce6563d4c184ecff3e34a0599fa2af8d6c65ef5
				if( (!stxData.empty()) && (stxData.find(strBitNetLotteryMagic) == 0) )	//"BitNet Lottery:"
				{
					...
					{
						i++;
						if( i == 8 ){ sEndBlock = pch; iEndBlock = strToInt64(pch); }
...
Value gettransactionblockheight(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return block num  about <txid>.");

    string sTx = params[0].get_str();
	uint64_t i = GetTransactionBlockHeight(sTx);
...
Value getprivkeysaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return <privkey>'s luckchain address.");
...
bool isValidLotteryGenesisTx(const CTransaction& tx, int iTxHei, int iTargetGuessType, int iTargetGuessLen, int64_t i6TargetBlock, const string sTargetMaker, const string sTargetGenesisAddr)
{
	...
	if( isBitNetLotteryRuleStart() ){ i6Mini = BitNet_Lottery_Create_Mini_Amount; }
	if( iAmount < i6Mini )
	{
					...
					BOOST_FOREACH(const CTxDestination& addr, addresses)
					{
						...
						{ 
							...
							if( entry != NULL )
							{
								...
								string sBetAndTxt = strprintf("%dBASH, bet txt '%s'", (int)(txout.nValue / COIN), sBetTxt.c_str());
																  ...
																  int64_t v_TargetValue, string& sRztBiggerAddr, int64_t& i6RztBiggerValue)
{
	int rzt = 0;
	if( fDebug ){
		printf("getBetBiggerWinnerFromBlockRange: [%u] ~ [%u], Guess Len [%u], Type [%u], Answer [%s], Genesis Addr [%s], Target Amount [%I64u]BASH \n", 
...
{
	int rzt = -1;
	string sTxMsg = tx.chaindata.c_str();  // BitNet Freeze|60|Vxxxxx..........|Target Address
...
{
	bool rzt = false;
	//if( (iPreTxHei == 0) || (txPrev.chaindata.length() < 13) ){ return rzt; }
	//string sTxMsg = txPrev.chaindata.c_str();  // BitNet Freeze|60|Vxxxxx..........|Remark
	//if( sTxMsg.find(str_BitNet_Freeze) != 0 ){ return rzt; }   //std::string::npos
...
{
	int rzt = 0;
	if( tx.chaindata.length() < 55 ){ return rzt; }
	string stxData = tx.chaindata.c_str();
	char * pVpn = (char *)stxData.c_str();
	char *pch;   char *delim = "/";
	pch = strtok(pVpn, delim);
...
void DomainService(const CTransaction& tx, uint64_t iTxHei, const string sTxMsg, const string sTxHash)
{
...
Value resubmitbitchaintx(const Array& params, bool fHelp)
{
    ...
    string sTx = params[0].get_str();
	CTransaction tx;
	if( GetTransactionByTxStr(sTx, tx) > 0 )
	{
		string sTxHash = tx.GetHash().ToString();
		string sTxMsg = tx.chaindata;
...
bool isRejectTransaction(const CTransaction& tx, uint64_t iTxHei)
{
	bool rzt = false;
	...
	{ 
#ifdef WIN32
		if( fDebug )printf("isRejectTransaction: Enash tx's hei = [%I64u], set to nBestHeight [%u] \n", iCashTxHei, nBestHeight); 
#endif
		iCashTxHei = nBestHeight;  
	}
	string sTxMsg = tx.chaindata;
	if( fDebug ){ printf("isRejectTransaction: Tx hei = [%s], nBestHeight [%s], \n\t Tx Msg = [%s] \n", u64tostr(iCashTxHei).c_str(), u64tostr(nBestHeight).c_str(), sTxMsg.c_str()); }
    ...
    if( !acceptBitBetTx(tx, iTxHei) ){ return true; }
...
Value getlotterywinner(const Array& params, bool fHelp)
{
	...
	//if( i > 0 )
	{
		...
		CTransaction tx;
		if( GetTransactionByTxStr(sId, tx) > 0 )
		{
...
Value sendtoaddresswithmsg(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddresswithmsg <luckchainaddress> <amount> [message] [encrypt message]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
int gettxbymsgFromBlock(unsigned int nHeight, const string sAddr, const string sMsg, int64_t iAmount, string& sRzt)
{
	...
	if( pblockindex )
	{
		...
		BOOST_FOREACH(const CTransaction& tx, block.vtx)
		{
			if( (iMsgLen > 0) && (tx.chaindata.length() == 0) ){ continue; }
...
std:: string aesencode(const string sPlain, string sKey)
{
	string rzt = "", sMi="";
	int iLen = sKey.length();
	if( iLen < 32 ){ sKey = GetAddressesByAccount(sBitChain_key, true); }
	iLen = sPlain.length();
    AES aes(Bits256, (unsigned char*)sKey.c_str());
	int i = aes.Cipher(sPlain, sMi);
	rzt = EncodeBase64((unsigned char*)sMi.c_str(), sMi.length());
	return rzt;
}
...
std::string aesdecode( const string sClipher, string sKey)
{
	string rzt = "";
	if( sKey.length() < 32 ){ sKey = GetAddressesByAccount(sBitChain_key, true); }  // sBitChainKeyAddress; }
	string ss = DecodeBase64(sClipher);
    AES aes(Bits256, (unsigned char*)sKey.c_str());
	int i = aes.InvCipher(ss, rzt);
	return rzt;
}
...
Value aesdecode(const Array& params, bool fHelp)
{
	...
	string sYuan = aesdecode(sTxt, sKey);
	return sYuan;
}
