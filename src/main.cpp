...
//dbLuckChainWriteSqlBegin( true );   // 2016.10.14 add
	bool bRej = isRejectTransaction(tx, 0);
//dbLuckChainWriteSqlBegin( false );   // 2016.10.14 add
	if( bRej )  //if( (isSyncBlockMode() == false) && isRejectTransaction(tx, 0) )
	{ 
...
bool CTransaction::CheckTransaction(bool bChkMiniValue) const
{
    ...
    int64_t nValueOut = 0;      bool fBitBetEncashTx = isBitBetEncashTx(*this);
                        ...
                        bool* pfMissingInputs)
{
    ...
    bool fBitBetEncashTx = isBitBetEncashTx(tx);  // will check details in IsStandardTx() -> acceptBitBetTx();
    if( fDebug ){ printf("AcceptToMemoryPool : fBitBetEncashTx=[%d]\n", fBitBetEncashTx); }
...

    {
        ...
        if (!tx.ConnectInputs(txdb, mapInputs, mapUnused, CDiskTxPos(1,1,1), pindexBest, false, false, fBitBetEncashTx))
        {
...
CBlockIndex* FindBlockByHeight(int64_t nHeight)
{
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if( pindexBest->nHeight < 10 )	//nBestHeight
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%s  trust=%s  blocktrust=%"PRId64"  date=%s\n",
      pindexNew->GetBlockHash().ToString().substr(0,20).c_str(), u64tostr(pindexNew->nHeight).c_str(),
      CBigNum(pindexNew->nChainTrust).ToString().c_str(), nBestInvalidBlockTrust.Get64(),
      DateTimeStrFormat("%x %H:%M:%S", pindexNew->GetBlockTime()).c_str());
    printf("InvalidChainFound:  current best=%s  height=%s  trust=%s  blocktrust=%"PRId64"  date=%s\n",
      hashBestChain.ToString().substr(0,20).c_str(), u64tostr(nBestHeight).c_str(),
...
bool CTransaction::ConnectInputs(CTxDB& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx,
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fBitBetEncashTx)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                bool bRt =  true;     CDiskTxPos& dtp = txindex.vSpent[prevout.n];      CTransaction txPay;
				if( fDebug && (txPay.ReadFromDisk(dtp)) )
				{
					...
					{
						bool bok = isValidBitBetEncashTx(txPay, iPayTxHei, bbp);
						if( fDebug ){ printf("ConnectInputs() : isValidBitBetEncashTx()=[%d] \n", bok); }
					}
				}
				if( bRt ){ return fMiner ? false : error("ConnectInputs() : %s prev tx [%s] already used at %s", GetHash().ToString().substr(0,10).c_str(), txPrev.GetHash().ToString().c_str(), txindex.vSpent[prevout.n].ToString().c_str()); }
            }
            ...
            {
                // Verify signature
                if( (!fBitBetEncashTx) && (!VerifySignature(txPrev, *this, i, 0)) )  //if (!VerifySignature(txPrev, *this, i, 0))
                {
        ...
        if( !fBitBetEncashTx && !IsCoinStake() )
        {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int64_t nFees = 0;
    int64_t nValueIn = 0;
    int64_t nValueOut = 0, nBurnedCoins = 0;
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            //int bitBetType = getBitBetTxType(tx);      bool fBitBetEncashTx = (bitBetType == 3);
			uint64_t u6ToMinerFee=0;      bool fBitBetEncashTx = isBitBetEncashTx(tx, u6ToMinerFee);
			//if( (bitBetType > 0) && (bitBetType < 3) )
			{
				GetTxOutBurnedCoins(tx.vout, nBurnedCoins, false);
			}
            ...
            if (!tx.IsCoinStake())
            {
                if( fBitBetEncashTx )
				{
					nFees += (u6ToMinerFee * COIN);   /*double d2 = (double)nTxValueOut * BitBet_RewardMiner_Rate;
					int64_t nTxFees = (int64_t)d2;
					if( nTxFees < COIN ){ nTxFees = COIN; }
					else{ uint64_t u6m = nTxFees % COIN;   if( u6m >= (COIN / 2) ){ nTxFees += COIN; } }
					nFees += nTxFees;
					//if( nTxValueOut >= BitBet_MiniTxOutValue_For_RewardMiner ){ nTxFees = (int64_t)((double)(nTxValueOut * BitBet_RewardMiner_Rate)); }  // 100000 COINS,   0.001
#ifdef WIN32
					if( fDebug ){ printf("ConnectBlock : nTxValueOut=[%I64u], nTxFees=[%I64u] [%s] \n", nTxValueOut, nTxFees, hashTx.ToString().c_str()); }
#endif  */
				}
    ...
    pindex->nMoneyBurned = (pindex->pprev? pindex->pprev->nMoneyBurned : 0) + nBurnedCoins;
...
dbLuckChainWriteSqlBegin( 0 );   // 2016.10.14 add
    ...
    {
        if( fDebug ){ printf("\nSetBestChain() : (hashPrevBlock != hashBestChain) : ( \n"); }
  ...
  /*else{
	...
	printf("Stake checkpoint: %X, height=%s\n", pindexBest->nStakeModifierChecksum, u64tostr(pindexBest->nHeight).c_str());
...
/*    uint64_t u6Time = 0;   if( fDebug ){ u6Time = GetTimeMillis(); }
    syncAllBitBets( nHeight );   // 2016.09.08 add
    uint64_t u6Time2 = 0;   if( fDebug ){ u6Time2 = GetTimeMillis() - u6Time; }
	if( fDebug ){ printf("\n\nAcceptBlock() : syncAllBitBets() used time = [%s] \n", u64tostr(u6Time2).c_str()); }
    if( fDebug ){ u6Time = GetTimeMillis(); }
int vtxSz = vtx.size();      bool bRejTx = false,  bSqlBoostMode = vtxSz >= 20;
if( bSqlBoostMode ){ dbLuckChainWriteSqlBegin( true ); }   // 2016.10.14 add
	...
	{			
		bRejTx = isRejectTransaction(tx, nHeight);
		if( bRejTx ){
			mempool.remove(tx);     //mempool.removeConflicts(tx);
			break; 
		}
	}
if( bSqlBoostMode ){ dbLuckChainWriteSqlBegin( false ); }  // 2016.10.14 add
    if( fDebug ){ u6Time2 = GetTimeMillis() - u6Time; }
	if( fDebug ){ printf("AcceptBlock() : vtx.size() = [%d], isRejectTransaction() used time = [%s] \n\n", vtxSz, u64tostr(u6Time2).c_str()); }
		if( bRejTx ){ return DoS(10, error("AcceptBlock() : block includes not under rule's tx, ban.")); }
#ifdef QT_GUI
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        ...
        {
			unsigned int nT2 = max(pindexBest->GetPastTimeLimit()+1, PastDrift(pindexBest->GetBlockTime(), pindexBest->nHeight+1));
            //if( fDebug ){ printf("CBlock::SignBlock : wallet.CreateCoinStake() = true, txCoinStake.nTime=[%u :: %u] \n", txCoinStake.nTime, nT2); }
			if( txCoinStake.nTime >= nT2 )  //if (txCoinStake.nTime >= max(pindexBest->GetPastTimeLimit()+1, PastDrift(pindexBest->GetBlockTime(), pindexBest->nHeight+1)))
            {
        ...
        }//else if( fDebug ){ printf("CBlock::SignBlock : wallet.CreateCoinStake() = false \n"); }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LuckChain", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        CBlock block;
        block.blockData = "People all around the world love to bet";
...
unsigned char pchMessageStart[4] = { 0x42, 0x41, 0x53, 0x48 };   // BASH
extern int fNetDbg;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	else if (strCommand == "version")
    {
        ...
        if( (pfrom->nVersion < MIN_PEER_PROTO_VERSION) || (pfrom->nVersion == 60014) )
        {
        ...
        if (!vRecv.empty())
		{
            uint64_t u6;     vRecv >> u6;      pfrom->nStartingHeight = u6;
		}
...
//--2014.11.10 end
		...
		printf("receive version message: version %d, zipblock=%d, Network_id=%d, BitVer=%d, blocks=%s, us=%s, them=%s, peer=%s, %u, %u:%u\n", pfrom->nVersion, pfrom->zipblock, pfrom->vBitNet.v_Network_id, pfrom->vBitNet.v_iVersion, u64tostr(pfrom->nStartingHeight).c_str(), addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), pfrom->vBitNet.v_ListenPort, pfrom->vBitNet.v_iVpnServiceCtrlPort, pfrom->vBitNet.v_P2P_proxy_port);
...
#ifdef USE_BITNET 
				uint64_t iHi = GetTransactionBlockHeight(sBonusId);
...
if( fDebug ){ printf("Node %s Network_id [%d] diff with [%d]\n", pfrom->addr.ToString().c_str(), pfrom->vBitNet.v_Network_id, BitNet_Network_id); }
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload());  // LuckChain: immediate retry during initial download
    ...
    else if (strCommand == "getblocks")
    {
        ...
        if (pindex)
            pindex = pindex->pnext;
        int nLimit = 500;
        if( fDebug ){ printf("getblocks %"PRId64" to %s limit %d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str(), nLimit); }
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            if (--nLimit <= 0)
            {
                ...
                printf("  getblocks stopping at limit %"PRIu64" %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
	...
	else if (strCommand == "getblockv2")  // 2015.07.24 add
	{
		...
		uint256 hashStop = 0;
        if( fDebug ){ printf("getblockv2 %"PRIu64" to %s limit %d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str(), nLimit); }
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
            ...
            if (--nLimit <= 0)
            {
                ...
                if( fDebug ){ printf("  getblockv2 stopping at limit %"PRIu64" %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str()); }
                pfrom->hashContinue = pindex->GetBlockHash();
                break;
            }
        }
