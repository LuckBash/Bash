...
// Copyright (c) 2014 The LuckChain developers
...
#endif */
	//std::string s = strprintf("%d, prev = [%X]", pindex->nHeight, pindex->pprev);  //hs.ToString();
	uint256 hashStop = pindex->GetBlockHash();
//printf("GetStakeModifierChecksum hi %d, pindex->pprev [%d] [%s]\n", pindex->nHeight, pindex->pprev, hashStop.ToString().c_str());
