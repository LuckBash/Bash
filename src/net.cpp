	...
	try{
		while( !fShutdown )
		{
			...
			if( (bUpdate_price_now > 0) || ((i62 - i61) > 180) )   // 3 minutes sync once
			{
				...
				string sUrl = "/bash2cny.php?tm=" + sTm;
...
void CNode::PushVersion()
{
    ...
    int64_t nTime = (fInbound ? GetAdjustedTime() : GetTime()),  bh = nBestHeight;
	...
	*/			
    ...
    this->ssSend << nLocalHostNonce << FormatSubVersion(CLIENT_NAME, CLIENT_VERSION, std::vector<string>()) << bh << BitNet_Version;
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (FD_ISSET(pnode->hSocket, &fdsetRecv) || FD_ISSET(pnode->hSocket, &fdsetError))
            {
                ...
                if (lockRecv)
                {
                    unsigned int iTrs, iRfs = 1024 * GetArg("-maxreceivebuffer", 11*1024);
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "LuckChain " + FormatFullVersion();
...
static const char *strDNSSeed[][2] = {
    {"s1.luckchain.org", "seed.luckchain.org"},
    {"s2.luckchain.org", "node.luckchain.org"},
    {"s3.luckchain.org", "pool.luckchain.org"},
	{"s4.luckchain.org", "s4.luckchain.org"},
	{"s5.luckchain.org", "s5.luckchain.org"},
	{"s6.luckchain.org", "s6.luckchain.org"},
	{"s7.luckchain.org", "s7.luckchain.org"},
	{"s8.luckchain.org", "s8.luckchain.org"},
	{"s9.luckchain.org", "s9.luckchain.org"},	
	{"sa.luckchain.org", "abe.luckchain.org"},
	{"sf.luckchain.org", "faucet.luckchain.org"},
...
void ThreadDNSAddressSeed(void* parg)
{
    ...
    RenameThread("luckchain-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("luckchain-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("luckchain-opencon");
...
{
    // Make this thread recognisable as the connection opening thread
    RenameThread("luckchain-openone");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("luckchain-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. LuckChain is probably already running."), addrBind.ToString().c_str());
        else
...
#ifdef QT_GUI
    if( GetBoolArg("-getprice", false) ){ NewThread(ThreadGetVpnPrice, NULL); }  // 2016.02.03 add
    if( GetBoolArg("-loginbitchain", false) ){ NewThread(ThreadWalletLoginDns, NULL); }  // 2016.04.02 add
#endif
